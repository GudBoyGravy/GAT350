...
void Framebuffer::Clear(const color_t& color)
{
    ...
    {
        ((color_t*)(uint32_t*)((void*)colorBuffer.data))[i] = color;
    }
}
...
{
    DrawLine(x, y, x + rect_width, y, color);
    DrawLine(x, y, x, y + rect_height, color);
    DrawLine(x, y + rect_height, x + rect_width, y + rect_height, color);
    DrawLine(x + rect_width, y + rect_height, x + rect_width, y, color);
}
...
void Framebuffer::DrawCubicCurve(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int steps, const color_t& color)
{
    ...
    for (int i = 0; i < steps; i++) 
    {
        ...
        float b1 = (float)pow(3 * (1.0f - t1), 2.0f);
        float c1 = (float)pow(3 * (1.0f - t1), 2.0f);
        ...
        float b2 = (float)pow(3 * (1.0f - t2), 2.0f);
        float c2 = (float)pow(3 * (1.0f - t2), 2.0f);
...
{
    for (uint32_t y = 0; y < image->colorBuffer.height; y++)
    {
        int sy = y1 + y;
        for (uint32_t x = 0; x < image->colorBuffer.width; x++)
        {
            int sx = x1 + x;
            ...
            //uint32_t index = x + (y * image->colorBuffer.width);

            //((color_t*)colorBuffer.data)[sx + (sy * colorBuffer.width)] = ((color_t*)image->colorBuffer.data)[index];////image->colorBuffer.data[index]
        }
    }
}
